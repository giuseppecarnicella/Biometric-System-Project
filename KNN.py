...
# Train and return the best KNN model
...
def identify_user(model, samples ,threshold=0.5):
    ...
    subject = model.predict(samples)  
    proba = np.max(model.predict_proba(samples),axis=1) 
    subject = np.where(proba <= threshold, 'Unknown', subject) # If the probability is lower than the threshold, the user is unknown
...
def identificate_all():
    ...
    data, y,_,_ = load_data(Dataset, ['subject', 'sessionIndex', 'rep'])
    Y = pd.get_dummies(y).values
    X_test,_ = split_data(data, y)
...
'''
data, y,data_imposter,y_imposter=load_data(Dataset, ['subject', 'sessionIndex', 'rep'])
